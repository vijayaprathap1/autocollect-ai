...
- Google OAuth configuration already exists in `apps/api/src/lib/oauth.ts` (googleEnabled, googleAuthorizeUrl, googleExchangeCode, etc.)
- Environment variables already exist in `apps/api/src/config.ts` and `.env.example`
- No Google OAuth controller/routes exist yet
- No frontend Google Sign-in buttons exist yet

## Task 1: Create Google OAuth Controller
...
## Task 2: Integrate Google OAuth into Auth Controller
...
## Task 3: Add Google Sign-in Button to Frontend
...
*(Note: Google OAuth configuration already exists in oauth.ts, config.ts, and .env.example)*
